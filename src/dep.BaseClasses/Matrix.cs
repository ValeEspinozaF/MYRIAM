...
namespace DataStructures
{
    public partial class Matrix
    {
        ...
        public static double[,] Inverse(double[,] matrix)
        {
            ...
            int[] permut;
            double[,] LUP = Decompose(matrix, out permut);
            if (LUP == null)
        ...
        {
            double[,] result = Create(matrix.GetLength(0), matrix.GetLength(1));
        ...
        public static double[,] Decompose(double[,] matrix, out int[] permut)
        {
            int rows = matrix.GetLength(0);
            int cols = matrix.GetLength(1);
            if (rows != cols)
